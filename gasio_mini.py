...
if file_usage and file_master:
    ...
    if df_usage is not None and df_master is not None:
        ...
        # 【改修】マルチセレクトに変更
        selected_ids = st.multiselect("分析対象の料金表番号を選択 (複数で合算)", usage_ids, default=usage_ids[:1])
        ...
        for tid in selected_ids:
            ...
            if not m_sub.empty:
                # MAX値をソートしたリストを取得し、末尾(上限)を固定して比較
        ...
        # グラフ描画ガード (データが空の場合のTypeError回避)
        if not agg_df.empty and total_count > 0:
            with g1:
                st.plotly_chart(px.pie(agg_df, values='調定数', names='Current_Tier', hole=0.5, color_discrete_sequence=chic_colors, title="調定数シェア", sort=False), use_container_width=True)
            with g2:
                st.plotly_chart(px.pie(agg_df, values='総使用量', names='Current_Tier', hole=0.5, color_discrete_sequence=chic_colors, title="使用量シェア", sort=False), use_container_width=True)
