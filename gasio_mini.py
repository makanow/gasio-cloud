...
def get_standard_tier_label(usage, sorted_master):
    ...
    # usageが0の場合や境界値そのものの場合の判定を安定化
    applicable = sorted_master[sorted_master['MAX'] >= round(usage, 4)]
...
if file_usage and file_master:
    ...
    # ソート順の付与
    labels_in_order = [get_standard_tier_label(r['MAX'] - 0.0001, master_rep) for _, r in master_rep.iterrows()]
    ...
    # 【重要】データが0の場合はグラフを描画しないガード
    if total_count > 0 and total_vol > 0:
        g1, g2 = st.columns(2)
        ...
        st.warning("表示できる集計データがありません（調定数または使用量が0です）。")
