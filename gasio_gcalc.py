...
st.set_page_config(page_title="G-Calc Master: 精密調整版", layout="wide")
st.title("🛡️ G-Calc Cloud: 投資・償却資産 統合算定エンジン")

EXCEL_FILE = "G-Calc_master.xlsx"

# 【精密座標】列4(index 3)から資産が並ぶ
ASSET_CONFIG = {
    "建物": {"col": 3, "code": "TTM"},          # 列4
    "構築物": {"col": 4, "code": "KCB"},        # 列5
    "集合装置": {"col": 5, "code": "SGS"},      # 列6
    "容器": {"col": 6, "code": "YKI"},          # 列7
    "導管・鋼管共同": {"col": 7, "code": "DKK"}, # 列8
    "導管・ＰＥ共同": {"col": 8, "code": "DPK"}, # 列9
    "導管・鋼管単独": {"col": 9, "code": "DKT"}, # 列10
    "導管・ＰＥ単独": {"col": 10, "code": "DPT"}, # 列11
    "メーター": {"col": 11, "code": "MTR"},      # 列12
}
...
def load_infra_master():
    try:
        ...
        # A. 償却率の取得（2行目/index 1、列4/index 3以降）
        rates = pd.to_numeric(df_raw.iloc[1, 3:12], errors='coerce').fillna(0).tolist()
        
        # B. 単価データの取得（4行目/index 3以降から実際のデータが始まると想定）
        df_data = df_raw.iloc[2:].copy()
        # 列1(index 1)に「HK」が含まれる行を抽出
...
for idx, row in edited_df.iterrows():
    ...
    if p_data is not None:
        raw_val = p_data.iloc[cfg["col"]]
        try:
            u_price = float(raw_val) # ここで文字ならエラーを投げてexceptへ
        except:
            u_price = 0
