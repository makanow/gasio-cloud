...
# 1. ページ構成
# ---------------------------------------------------------
st.set_page_config(page_title="G-Calc Master: 投資算定エンジン", layout="wide")
st.title("🛡️ G-Calc Cloud: 投資・償却資産算定（意思決定支援モード）")
...
if not res_df.empty:
    ...
    m1.metric("投資額① (非減免合計)", f"¥ {res_df['投資額①'].sum():,.0f}")
    m2.metric("投資額② (減免合計)", f"¥ {res_df['投資額②'].sum():,.0f}")
