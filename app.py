...
st.markdown('<div class="sub-title">Cloud Edition - Unified Analysis System</div>', unsafe_allow_html=True)
...
if file_usage and file_master and selected_ids:
    ...
    with t2:
        ...
        if st.session_state.simulation_result is not None:
            ...
            summ = [{"プラン名":"現行", "売上":total_curr, "差額":0}]
            for pn in new_plans:
                t_new = sr[pn].sum()
                summ.append({"プラン名":pn, "売上":t_new, "差額":t_new - total_curr})
            st.dataframe(pd.DataFrame(summ).style.format({"売上":"¥{:,.0f}", "差額":"¥{:,.0f}"}), hide_index=True, use_container_width=True)
    ...
    with t3:
        ...
        with g1:
            ...
            if ids_consistent:
                ...
                st.plotly_chart(px.pie(agg_c, values='調定数', names='現行区画', hole=0.5, color_discrete_sequence=CHIC_PIE_COLORS, title="現行：調定数シェア"), use_container_width=True, key="pie_c")
                st.dataframe(agg_c, hide_index=True)
            else:
                st.warning("⚠️ 境界不一致。現行の合算グラフは表示できません。")

        with g2:
            st.markdown(f"**Proposal: {sel_p}**")
            # 新プランは常に単一構造なので、合算判定に関係なく常に表示する
            if sel_p in new_plans:
                ...
                st.plotly_chart(px.pie(agg_n, values='調定数', names='新区画', hole=0.5, color_discrete_sequence=CHIC_PIE_COLORS, title=f"{sel_p}：調定数シェア"), use_container_width=True, key="pie_n")
