...
# [load_ratemake_format などの特殊ロジックもすべて維持]
...
if file_usage and file_master and selected_ids:
            ...
            with pt:
                ...
                with c2:
                    if not edited.empty:
                        ...
                        st.plotly_chart(fig, use_container_width=True, key=f"prev_chart_{i}")
    ...
    with tab2:
        ...
        if st.session_state.simulation_result is not None:
            ...
            summ = [{"プラン名":"現行", "売上総額":total_curr, "差額":0}]
            for pn in new_plans:
                t_new = sr[pn].sum()
                summ.append({"プラン名":pn, "売上総額":t_new, "差額":t_new - total_curr})
    ...
    with tab3:
        ...
        with g1:
            ...
            if len(set(fps.values())) <= 1:
                ...
                st.plotly_chart(px.pie(agg_c, values='調定数', names='現行区画', hole=0.5, color_discrete_sequence=CHIC_PIE_COLORS, title="現行：調定数シェア"), use_container_width=True, key="ana_pie_c")
                st.dataframe(agg_c, hide_index=True)
            else:
                st.info("複数料金合算のためヒストグラムを表示")
                st.plotly_chart(px.histogram(df_target_usage, x="使用量", color="料金表番号", nbins=50, color_discrete_sequence=CHIC_PIE_COLORS, title="全体使用量分布"), use_container_width=True, key="ana_hist_c")
        with g2:
            st.markdown(f"**Proposal: {sel_p}**")
            if sel_p in new_plans:
                ...
                st.plotly_chart(px.pie(agg_n, values='調定数', names='新区画', hole=0.5, color_discrete_sequence=CHIC_PIE_COLORS, title=f"{sel_p}：調定数シェア"), use_container_width=True, key="ana_pie_n")
                st.dataframe(agg_n, hide_index=True)
else:
