...
with c_head1:
    ...
    st.markdown('<div class="sub-title">Cloud Edition - Executive Rate Simulation System</div>', unsafe_allow_html=True)
...
# ---------------------------------------------------------
# 2. 関数定義 (RateMake特殊読み込みロジック含む完全復元)
# ---------------------------------------------------------
...
def get_pdf_report_html(summ_df):
    ...
    <body>
        <div class="header"><div class="title">Gasio 料金改定シミュレーション 報告書</div></div>
        <p>生成日時: {datetime.datetime.now().strftime('%Y/%m/%d %H:%M')}</p>
        <h3>■ 収支インパクト計</h3>
        {summ_df.to_html(index=False, classes='table')}
        <p style="margin-top: 20px; color: #7f8c8d;">※本ページを右クリック→「印刷」からPDFとして保存してください。</p>
    </body>
    </html>
    """
    b64 = base64.b64encode(html.encode()).decode()
    return f'<a href="data:text/html;base64,{b64}" target="_blank" style="text-decoration:none;"><button style="width:100%; height:40px; background-color:#e74c3c; color:white; border:none; border-radius:4px; cursor:pointer; font-weight:bold;">📄 報告用PDFレポートを即時生成</button></a>'
...
if file_usage and file_master and selected_ids:
            ...
            with pt:
                ...
                with c2:
                    if not edited.empty:
                        ...
                        res_df = pd.DataFrame([{"区画名":r['区画名'], "MIN":0, "MAX":r['適用上限(m3)'], "基本料金":bases.get(r['No'],0), "単位料金":r['単位料金']} for _, r in edited.iterrows()])
    ...
    with tab_sim:
        ...
        if st.session_state.simulation_result is not None:
            ...
            with gc2: st.plotly_chart(px.scatter(sr.sample(min(len(sr),1000)), x='使用量', y=['現行料金', sel_p], title="新旧料金プロット(サンプル1000件)", opacity=0.6), use_container_width=True)
            ...
            with c_pdf:
                st.markdown(get_pdf_report_html(pd.DataFrame(summ_list)), unsafe_allow_html=True)
