...
def get_pdf_report_html(summ_df, sel_plan_name):
    ...
    <head>
        <meta charset="UTF-8">
        <style>
            body {{ font-family: "Helvetica Neue", Arial, "Hiragino Kaku Gothic ProN", "Hiragino Sans", Meiryo, sans-serif; padding: 50px; color: #2c3e50; line-height: 1.6; }}
            ...
            th {{ background-color: #f8f9fa; padding: 12px; border: 1px solid #dee2e6; text-align: center; }}
    ...
    <body>
        <div class="report-header">
            <h1 class="report-title">ガス料金改定シミュレーション報告書</h1>
            <span class="date">作成日: {datetime.datetime.now().strftime('%Y年%m月%d日')}</span>
        </div>
        <div class="section-title">1. 収支インパクト計</div>
        <p>「{sel_plan_name}」を主対象としたシミュレーション結果は以下の通りです。</p>
        {formatted_table}
        <div class="footer" style="margin-top:50px; text-align:center; font-size:12px; color:#95a5a6;">Gasio Cloud Edition Report</div>
        <div class="no-print" style="margin-top:30px; text-align:center;">
            <button onclick="window.print()" style="padding:10px 20px; background-color:#3498db; color:white; border:none; border-radius:5px; cursor:pointer; font-weight:bold;">PDFとして保存 / 印刷</button>
        </div>
    </body>
    </html>
    """
    try:
        ...
        return f'<a href="data:text/html;charset=utf-8;base64,{b64}" target="_blank" style="text-decoration:none;"><button style="{button_style}">📄 報告用レポートを生成</button></a>'
    except:
        return "レポート生成エラー"
...
if file_usage and file_master and 'selected_ids' in locals():
    ...
    with t_sim:
        ...
        if st.session_state.simulation_result is not None:
            ...
            with gc2: st.plotly_chart(px.scatter(sr.sample(min(len(sr),1000)), x='使用量', y=['現行料金', sel_p], title="新旧プロット(1000件)", opacity=0.6), use_container_width=True)
            ...
            with c_ex:
                ...
                st.download_button("📊 全件明細Excelを保存", data=output.getvalue(), file_name=f"Gasio_Simulation_{datetime.datetime.now().strftime('%Y%m%d')}.xlsx", use_container_width=True)
            with c_pdf:
