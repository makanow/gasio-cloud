...
st.markdown('<div class="sub-title">Rate Design Solver (Final Robust Build)</div>', unsafe_allow_html=True)
...
with tab1:
    st.info("💡 行を追加すると、即座にNoと区画名が確定します。")
    ...
    with c1:
        ...
        # エディタの実行
        # point: data_editor に渡すDF自体を stabilize してから渡す
        ...
        # 変更があれば即座に再計算してステートを更新
        # rerunを最小限にするため、値が異なる場合のみ処理
    ...
    with c2:
        ...
        if not res_df.empty:
            ...
            st.dataframe(
                display_df.set_index('No').style.format("{:,.2f}"), 
