...
def get_sample_usage_csv():
    df = pd.DataFrame({
        '料金表番号': [10, 10, 10, 10, 10, 20, 20, 20, 20, 20, 30, 30, 30, 30, 30],
        '使用量': [5.2, 12.5, 28.0, 45.3, 3.0, 18.5, 35.2, 60.1, 14.0, 42.8, 55.0, 120.5, 80.2, 45.0, 210.0]
    })
...
def normalize_columns(df):
    ...
    for col in ['MIN', 'MAX', '基本料金', '単位料金']:
        if col in df.columns:
            ...
            if col == 'MAX': df[col] = df[col].fillna(999999999.0)
