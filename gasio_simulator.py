...
# 1. 設定 & デザイン (UIの純粋化)
# ---------------------------------------------------------
st.set_page_config(page_title="Gasio計算機", page_icon="🔥", layimport streamlit as st
import pandas as pd
import plotly.express as px
...
st.markdown("""
    ...
    .guide-box { background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 1rem; margin-bottom: 1rem; font-size: 0.85rem; }
    </style>
...
# ---------------------------------------------------------
# 4. メイン処理 (取り付け数等の不要カラムを排除)
# ---------------------------------------------------------
if file_m and file_u:
    df_master_all = pd.read_csv(file_m)
    df_usage_all = pd.read_csv(file_u)

    # 分析に必要なカラムのみに絞り込み (取り付け数などを削除)
    needed_cols = ['料金表番号', '使用量', '調定数']
    df_usage_all = df_usage_all[[c for c in needed_cols if c in df_usage_all.columns]]
    ...
    if selected_ids:
        ...
        with g1:
            ...
            m_rep = df_master_all[df_master_all['料金表番号'].astype(str) == selected_ids[0]].sort_values('MAX')
        ...
        st.subheader("📋 詳細データ (先頭100件)")
        st.dataframe(df_target_usage.head(100), use_container_width=True)
else:
    st.info("サイドバーからCSVファイルをアップロードしてください。")out="wide")
