...
# ---------------------------------------------------------
# 1. 設定 & デザイン (設定保存・復元ボタン関連のスタイルを廃止)
# ---------------------------------------------------------
...
if file_m and file_u:
    ...
    # 指示1: 使用量CSVから「調定数」以外の不要カラムを排除
    keep_cols = ['料金表番号', '使用量', '調定数']
    df_usage_all = df_usage_raw[[c for c in keep_cols if c in df_usage_raw.columns]].copy()
